...
public class AppUserRepositoryImpl implements AppUserRepository, PanacheRepositoryBase<AppUser, Long> {
    ...
    public void delete(UUID id) {
        this.update("SET deleted=true WHERE secureId=?1", id);
    }

    @Transactional
    @Override
    public void hardDelete(UUID id) {
        AppUser appUser= this.find("WHERE secureId=?1", id).firstResultOptional()
            .orElseThrow(()-> new ResourceNotFoundException("AppUser id not found"));
        this.delete(appUser);
    }
